...
- 尚未完成 PDF ingestion 到 LLM Wiki 的批次整理流程。
...
**影響與取捨**：目前只支援單一 PDF attach，不支援 Word、Excel、PowerPoint、圖片，也不做批次 ingestion。這樣功能較窄，但安全邊界清楚，後續 #12 的 LLM Wiki PDF ingestion 可以建立在這個基礎上。
...
**影響與取捨**：
#8 讓 tGenie 可以透過 Atlas 讀 workspace、搜尋檔案、寫檔，並執行受控 shell command。#9 讓 tGenie 可以透過 `pdf.attach` 上傳 workspace-local PDF。取捨是 shell confirmation 目前還沒有互動式確認 UI；需要確認的命令會回傳 `confirmation-required`，不會真的執行。單 PDF 到 LLM Wiki ingestion 仍屬於 #12。
