...
class TgenieAdapterTests(unittest.IsolatedAsyncioTestCase):
    def test_bootstrap_prompt_explains_atlas_tool_protocol(self) -> None:
        ...
        self.assertIn('"type": "atlas.tool_batch"', prompt)
        self.assertIn('"tool": "tool.name"', prompt)
        self.assertIn('"args": {}', prompt)
        self.assertIn("Request only one single tool call, or one `atlas.tool_batch`", prompt)
        self.assertIn("at most 5 calls", prompt)
        self.assertIn("The batch limit is not an inspection limit", prompt)
        self.assertIn("do not stop after one 5-call batch", prompt)
        self.assertIn("Do not put `file.write`, `shell.run`, or `file.attach` in a batch", prompt)
        self.assertIn(
        ...
        self.assertIn("atlas.tool_batch_result", prompt)
        self.assertIn("file.attach", prompt)
        self.assertIn(".jpg", prompt)
        self.assertIn(".png", prompt)
        self.assertIn("Completion discipline", prompt)
        self.assertIn("one useful single tool call or one useful read-only batch", prompt)
        self.assertIn("run targeted checks with `shell.run`", prompt)
        self.assertIn("Do not stop after the first file, first search result, or first tool result", prompt)
